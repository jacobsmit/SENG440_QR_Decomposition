...
int main() {
  ...
  for (int i = 0; i < 5; i++) {
    float true_angle = (ratio_tests_D[i] == 0.0f)
                           ? (PI_OVER_2_FIXED / 2048.0f)
                           : atanf(ratio_tests_N[i] / ratio_tests_D[i]);
    ...
    printf("N: %.1f, D: %.1f (Ratio: %.1f)\n", ratio_tests_N[i],
           ratio_tests_D[i],
           (ratio_tests_D[i] == 0.0f) ? 999.9f
                                      : ratio_tests_N[i] / ratio_tests_D[i]);
    printf("Standard atan(N/D): %f rad\n", true_angle);
    printf("calculate_arctan_ratio(): %d (Float: %f rad)\n\n", fixed_angle,
           FIXED_TO_FLOAT(fixed_angle));
  }
