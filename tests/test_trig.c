...
int main() {
    ...
    for (float angle = -0.786f; angle <= 0.786f; angle += 0.001f) {
        ...
        if (err_sin > max_err_sin) max_err_sin = err_sin;
    ...
    printf("Results:\n");
    printf("Max Sine Error:   %.4f (%.2f%%)\n", max_err_sin, max_err_sin * 100.0f);
    printf("Max Cosine Error: %.4f (%.2f%%)\n", max_err_cos, max_err_cos * 100.0f);
    ...
    printf("sin(0.5) = %.4f (True: %.4f)\n", FIXED_TO_FLOAT(sin_fixed(pt5)), sinf(0.5f));
    printf("cos(0.5) = %.4f (True: %.4f)\n", FIXED_TO_FLOAT(cos_fixed(pt5)), cosf(0.5f));
