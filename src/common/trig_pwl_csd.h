/* GENERATED by scripts/trig_approx_parameters/gen_csd_header.py -- do not edit.
 *
 * Firmware PWL tables. Slopes are constrained to three signed powers of two
 * so the microcoded engine, which has no multiplier, evaluates m*x as three
 * shift-adds. The constrained slope is still an exact integer, so (m*x)>>14
 * here is bit-identical to the assembly's shift-add chain and to the VHDL.
 *
 * SEPARATE from trig_pwl_tables.h on purpose: the real ARM has SMULL, so the
 * software variants keep exact slopes and their better accuracy. Only the
 * firmware and the hardware are multiplier-free. */
